...
const Citations = (() => {
    ...
    function decorate(html, indexMap) {
        ...
        return html.replace(/\[([SW])(\d+)\]/g, (whole, prefix, num) => {
            ...
            if (prefix === 'W') {
                ...
                if (!url) return whole;
                return `<sup class="weft-cite" data-source-url="${escapeAttr(url)}" data-cite="${key}" title="${title}">${key}</sup>`;
            }
            return `<sup class="weft-cite" data-snippet-id="${escapeAttr(meta.id)}" data-cite="${key}" title="${title}">${key}</sup>`;
        });
    }
    ...
    async function jumpToSource(snippetId) {
        ...
        const sourceUrl = SourceUtils.annotationSourceUrl(snippet);
        if (!sourceUrl) {
    ...
    function bindClicks(container) {
        ...
        container.addEventListener('click', (e) => {
            ...
            if (!chip) return;
            const externalUrl = chip.getAttribute('data-source-url');
            if (externalUrl) {
                openExternalSource(externalUrl);
                return;
            }
    ...
    return { CONTRACT, buildContext, decorate, jumpToSource, bindClicks, notify, safeExternalUrl };
})();
