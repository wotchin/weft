...
const SmartRead = (() => {
    ...
    function validateArticleAnalysis(raw, page) {
        ...
        for (let i = 0; i < scanCount; i++) {
            ...
            const evidenceIdentity = evidenceInput.slice(0, MAX_RAW_EVIDENCE)
                .map((candidate) => {
                    if (typeof candidate === 'string') {
                        return candidate.slice(0, LIMITS.evidenceQuoteMax + 1);
                    }
                    if (!isRecord(candidate)) return '';
                    return `${String(candidate.blockId || '').slice(0, 200)}:${String(candidate.quote || '')
                        .slice(0, LIMITS.evidenceQuoteMax + 1)}`;
                })
                .join('\n');
            const takeawayKey = fingerprint(
                `${title.toLowerCase()}\n${summary.toLowerCase()}\n${evidenceIdentity}`
            );
    ...
    function selectLinksForAnalysis(links, maxLinks = 80) {
        ...
        const limit = Number.isFinite(maxLinks)
            ? Math.max(0, Math.min(500, Math.floor(maxLinks)))
            : 0;
    ...
    function selectBlocksForAnalysis(blocks, maxChars) {
        ...
        // Short anchor blocks can leave budget behind. Add the midpoint of the
        // widest remaining interval. A small max-heap keeps this stratified
        // traversal O(n log n), instead of repeatedly scanning every selected
        // block for every candidate on pages with thousands of short blocks.
        const intervalHeap = [];
        const outranks = (a, b) => a.distance > b.distance
            || (a.distance === b.distance && a.index < b.index);
        const pushInterval = (left, right) => {
            ...
            const entry = {
                ...
                right,
                index,
                distance: Math.min(index - left, right - index),
            };
            intervalHeap.push(entry);
            let child = intervalHeap.length - 1;
            while (child > 0) {
                const parent = Math.floor((child - 1) / 2);
                if (!outranks(intervalHeap[child], intervalHeap[parent])) break;
                [intervalHeap[parent], intervalHeap[child]] = [intervalHeap[child], intervalHeap[parent]];
                child = parent;
            }
        };
        const popInterval = () => {
            if (intervalHeap.length === 0) return null;
            const first = intervalHeap[0];
            const last = intervalHeap.pop();
            if (intervalHeap.length > 0 && last) {
                intervalHeap[0] = last;
                let parent = 0;
                while (true) {
                    ...
                    const right = left + 1;
                    let next = parent;
                    if (left < intervalHeap.length && outranks(intervalHeap[left], intervalHeap[next])) {
                        next = left;
                    }
                    if (right < intervalHeap.length && outranks(intervalHeap[right], intervalHeap[next])) {
                        next = right;
                    }
                    if (next === parent) break;
                    [intervalHeap[parent], intervalHeap[next]] = [intervalHeap[next], intervalHeap[parent]];
                    parent = next;
                }
            }
            return first;
        ...
        const selectedIndices = [...chosen.keys()].sort((a, b) => a - b);
        for (let i = 1; i < selectedIndices.length; i++) {
            pushInterval(selectedIndices[i - 1], selectedIndices[i]);
        }

        while (remaining > 0 && chosen.size < valid.length) {
            const interval = popInterval();
            if (!interval) break;
            const nextIndex = interval.index;
            const text = sliceForPosition(
                valid[nextIndex].text,
                Math.min(valid[nextIndex].text.length, remaining),
                'start'
            );
            if (!text) break;
            chosen.set(nextIndex, exposedBlock(valid[nextIndex], text));
            remaining -= text.length;
            pushInterval(interval.left, nextIndex);
            pushInterval(nextIndex, interval.right);
        }

        return [...chosen.entries()].sort((a, b) => a[0] - b[0]).map((entry) => entry[1]);
    }

    function blockPayloadLength(blocks) {
        return JSON.stringify({
            blocks: blocks.map((block) => ({ id: block.id, text: block.text })),
        }).length;
    }

    function safeChunkEnd(text, start, hardEnd) {
        let end = Math.min(text.length, Math.max(start + 1, hardEnd));
        ...
        const softStart = start + Math.floor((end - start) * 0.6);
        for (let index = end; index > softStart; index--) {
    ...
    /**
     * Pack stable source blocks into serialized-size-bounded chunks. Normal
     * blocks stay intact; an oversized block keeps its original id and gets a
     * small overlap so a sentence crossing the cut can still become evidence.
     */
    function chunkBlocksForAnalysis(blocks, maxPayloadChars, overlapChars = 240) {
        const valid = validBlocks({ blocks: Array.isArray(blocks) ? blocks : [] });
        const budget = Number.isFinite(maxPayloadChars)
            ? Math.max(512, Math.floor(maxPayloadChars))
            : LIMITS.analysisChars;
        const overlapLimit = Number.isFinite(overlapChars)
            ? Math.max(0, Math.min(1000, Math.floor(overlapChars)))
            : 0;
        const chunks = [];
        let current = [];
        ...
        for (const block of valid) {
            const whole = exposedBlock(block, block.text);
            if (blockPayloadLength([...current, whole]) <= budget) {
                current.push(whole);
                continue;
            }
            flush();
            if (blockPayloadLength([whole]) <= budget) {
                current.push(whole);
                continue;
            }
            ...
            while (start < block.text.length) {
                ...
                while (low <= high) {
                    ...
                    const candidate = exposedBlock(block, block.text.slice(start, middle));
                    if (blockPayloadLength([candidate]) <= budget) {
                        bestEnd = middle;
                        low = middle + 1;
                    } else {
                        high = middle - 1;
                ...
                const end = bestEnd < block.text.length
                    ? safeChunkEnd(block.text, start, bestEnd)
                    : bestEnd;
                const text = block.text.slice(start, end);
                if (!text) break;
                chunks.push([exposedBlock(block, text)]);
                if (end >= block.text.length) break;
                const overlap = Math.min(overlapLimit, Math.floor((end - start) / 4));
                let nextStart = Math.max(start + 1, end - overlap);
                if (nextStart < block.text.length) {
                    const before = block.text.charCodeAt(nextStart - 1);
                    const after = block.text.charCodeAt(nextStart);
                    if (before >= 0xd800 && before <= 0xdbff && after >= 0xdc00 && after <= 0xdfff) {
                        nextStart--;
                    }
                }
    ...
    function linkPayloadLength(links) {
        return JSON.stringify({
    ...
    /** Keep link ids intact while packing crowded index pages into chunks. */
    function chunkLinksForAnalysis(links, maxPayloadChars) {
        const valid = [...knownLinks({ links: Array.isArray(links) ? links : [] }).values()];
        const budget = Number.isFinite(maxPayloadChars)
            ? Math.max(1000, Math.floor(maxPayloadChars))
            : LIMITS.analysisChars;
        const chunks = [];
        let current = [];
        for (const link of valid) {
            if (current.length > 0 && linkPayloadLength([...current, link]) > budget) {
                chunks.push(current);
                current = [];
            }
            current.push(link);
    ...
    function roundRobin(items, collectionName) {
        const collections = items.map((item) => item[collectionName] || []);
        const output = [];
        const longest = collections.reduce((max, collection) => Math.max(max, collection.length), 0);
        for (let index = 0; index < longest; index++) {
            for (const collection of collections) {
                if (collection[index]) output.push(collection[index]);
            }
        }
        return output;
    }

    /** Validate each map result, then deterministically merge across chunks. */
    function mergeArticleAnalyses(analyses, page) {
        const valid = (Array.isArray(analyses) ? analyses : [])
            .map((analysis) => validateArticleAnalysis(analysis, page))
            .filter((analysis) => analysis.takeaways.length > 0);
        const fallback = isRecord(page) && typeof page.title === 'string' ? page.title : DEFAULT_SESSION_TITLE;
        if (valid.length === 0) return validateArticleAnalysis({}, page);

        const combined = {
            sessionTitle: valid[0].sessionTitle || fallback,
            topic: valid.find((analysis) => analysis.topic)?.topic || fallback,
            takeaways: [],
        };
        const groups = [];
        for (const takeaway of roundRobin(valid, 'takeaways')) {
            const evidenceKeys = new Set(takeaway.evidence.map((evidence) =>
                fingerprint(normalizeForMatch(evidence.quote).toLowerCase())));
            const group = groups.find((candidate) => {
                const overlaps = [...evidenceKeys].some((key) => candidate.evidenceKeys.has(key));
                if (!overlaps) return false;
                const unionSize = new Set([...candidate.evidenceKeys, ...evidenceKeys]).size;
                return unionSize <= LIMITS.evidencePerTakeaway;
            });
            if (!group) {
                const copy = { ...takeaway, evidence: [...takeaway.evidence] };
                groups.push({ takeaway: copy, evidenceKeys });
                combined.takeaways.push(copy);
            } else {
                for (const evidence of takeaway.evidence) {
                    const key = fingerprint(normalizeForMatch(evidence.quote).toLowerCase());
                    if (group.evidenceKeys.has(key)) continue;
                    group.evidenceKeys.add(key);
                    group.takeaway.evidence.push(evidence);
                }
            }
        }
        return validateArticleAnalysis(combined, page);
    }

    function mergeIndexAnalyses(analyses, page) {
        const valid = (Array.isArray(analyses) ? analyses : [])
            .map((analysis) => validateIndexAnalysis(analysis, page))
            .filter((analysis) => analysis.selections.length > 0);
        const fallback = isRecord(page) && typeof page.title === 'string' ? page.title : DEFAULT_SESSION_TITLE;
        if (valid.length === 0) return validateIndexAnalysis({}, page);

        const seen = new Set();
        const selections = [];
        for (const selection of roundRobin(valid, 'selections')) {
            if (seen.has(selection.linkId)) continue;
            seen.add(selection.linkId);
            selections.push(selection);
        }
        return validateIndexAnalysis({
            sessionTitle: valid[0].sessionTitle || fallback,
            topic: valid.find((analysis) => analysis.topic)?.topic || fallback,
            selections,
        }, page);
    ...
    function buildSmartMetadata(options) {
        ...
        return {
            ...
            coverageLimited: opts.coverageLimited === true,
        };
    }
        ...
        for (let takeawayIndex = 0; takeawayIndex < analysis.takeaways.length; takeawayIndex++) {
            ...
            for (let evidenceIndex = 0; evidenceIndex < takeaway.evidence.length; evidenceIndex++) {
                ...
                snippets.push({
                    ...
                    smartReadCoverageLimited: meta.coverageLimited,
    ...
    return Object.freeze({
        ...
        chunkBlocksForAnalysis,
        chunkLinksForAnalysis,
        mergeArticleAnalyses,
        mergeIndexAnalyses,
