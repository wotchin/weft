 ...
 * Covers open, get, put, getAll, delete and bounded batch operations.
 */
/* exported WeftIDB */
...
const WeftIDB = (() => {
    ...
    function getAllKeys(db, storeName, indexName, query) {
        const { store } = _tx(db, storeName, 'readonly');
        const source = indexName ? store.index(indexName) : store;
        return _req(source.getAllKeys(query));
    }
    ...
    return { open, get, put, getAll, getAllKeys, delete: del, putAll, deleteByIndex };
})();
