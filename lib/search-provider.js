...
const SearchProvider = (() => {
    ...
    async function search(query, maxResults = 6, options = {}) {
        const cfg = await getConfig();
        if (options.signal?.aborted) throw abortError(options.signal);
        switch (cfg.provider) {
            case 'tavily': return tavily(query, maxResults, cfg, options.signal);
            case 'brave': return brave(query, maxResults, cfg, options.signal);
            case 'searxng': return searxngAt(cfg.endpoint, query, maxResults, options.signal);
            default: return [];
        }
    }
    ...
    // The consumer runs inside the same lifetime, so timeout/Stop also covers a
    // server that sends headers and then stalls while streaming the body.
    async function fetchWithTimeout(url, init = {}, callerSignal, consume) {
        const ctl = new AbortController();
        const abortFromCaller = () => ctl.abort(abortError(callerSignal));
        ...
        try {
            ...
            if (timedOut) {
                const timeoutError = new Error('The search provider did not respond in time.');
    ...
    async function tavily(query, maxResults, cfg, signal) {
        if (!cfg.apiKey) throw new Error('Tavily API key not set.');
        const data = await fetchWithTimeout('https://api.tavily.com/search', {
            method: 'POST',
    ...
    async function brave(query, maxResults, cfg, signal) {
     ...
     */
    async function searxngAt(endpoint, query, maxResults, signal) {
        ...
        try {
            ...
            if (e?.name === 'TimeoutError') throw new Error('The instance did not respond in time.');
            if (/^The instance |^That instance /u.test(String(e?.message || ''))) throw e;
            throw new Error('Could not reach that address. Check the URL and that the instance is online.');
        }
    ...
    return { search, getConfig, testConnection };
})();
