/**
 * PageExtractor — extract a webpage's rendered DOM or an opened HTTP(S) PDF.
 *
 * Webpages are read only from their rendered DOM. PDF tabs are handled by the
 * local PDFExtractor, which re-fetches the original PDF URL for text parsing.
 *
 * Returns: {
...
const PageExtractor = (() => {
     ...
     */
    async function extractFromTab(tabId, expectedUrl, options = {}) {
        ...
        const verifyNavigation = async (resultUrl = tabUrl) => {
            let latest;
            try {
            ...
            if (!isSameDocumentUrl(tabUrl, latestUrl)
                || (expectedUrl && !isSameDocumentUrl(expectedUrl, resultUrl))) {
                throw pageChangedError(expectedUrl || tabUrl, latestUrl || resultUrl);
            }
        };

        const extractPdf = async () => {
            const pdf = await PDFExtractor.extractFromUrl(tabUrl, {
                ...options,
                sourceTitle: options.sourceTitle || tab.title || '',
            });
            await verifyNavigation(pdf.url);
            return pdf;
        };
        ...
        const extractDom = async () => {
            ...
            await verifyNavigation(result.url);
            return result;
        };

        const hasUsefulDom = (result) =>
            (result?.content || '').trim().length >= 50 || (result?.links || []).length >= 3;
        const hasAnyDom = (result) =>
            Boolean((result?.content || '').trim() || (result?.links || []).length);
        const isTerminalError = (error) =>
            error?.name === 'AbortError' || error?.code === 'PDF_ABORTED'
            || error?.code === 'TARGET_PAGE_CHANGED' || error?.code === 'TARGET_TAB_UNAVAILABLE';

        const likelyPdf = PDFExtractor.isLikelyPdfUrl(tabUrl, tab.title || '');
        if (likelyPdf) {
            let pdfError = null;
            try {
            ...
            try {
                ...
                if (hasAnyDom(dom)) return dom;
            } catch (error) {
                if (isTerminalError(error)) throw error;
                domError = error;
            }
            if (pdfError?.code === 'PDF_NOT_PDF' && domError) throw domError;
            throw pdfError;
        }
        ...
        let domError = null;
        try {
            domResult = await extractDom();
            if (hasUsefulDom(domResult)) return domResult;
        } catch (error) {
            domError = error;
        }

        // Some PDF endpoints have no .pdf suffix. Chrome still keeps the
        // original HTTP(S) URL in the tab while its internal viewer owns the
        // text layer, so an empty/uninjectable DOM gets one safe PDF probe.
        try {
            return await extractPdf();
        } catch (error) {
            if (isTerminalError(error)) throw error;
            if (hasAnyDom(domResult)) return domResult;
            if (domError) {
                if (error?.code === 'PDF_NOT_PDF') throw domError;
                throw error;
            }
            return domResult;
        }
    ...
    async function extractAsSnippets() {
        ...
        const snippets = page.documentType === 'pdf'
            ? (page.blocks || []).flatMap((block, blockIndex) =>
                WeftTokenizer.chunkText(block.text || '', 512).map((chunk, chunkIndex) => ({
                    id: `page-${block.id || blockIndex}-${chunkIndex}`,
                    type: 'text',
                    content: chunk,
                    sourceUrl: page.url,
                    ...
                    timestamp,
                    tags: ['page-content', 'pdf'],
                })))
            : WeftTokenizer.chunkText(page.content, 512).map((chunk, i) => ({
                id: `page-chunk-${i}`,
                type: 'text',
                content: chunk,
                sourceUrl: page.url,
