...
const PageExtractor = (() => {
     ...
     */
    /**
     * Browser-internal and extension pages cannot be scripted, so page-based
     * features are unavailable there. Callers can use this to disable UI up
     * front instead of failing after the click.
     */
    function isRestrictedUrl(url) {
        return !url || /^(chrome|edge|brave|about|moz-extension|chrome-extension|devtools|view-source):/i.test(url)
            || url.startsWith('https://chrome.google.com/webstore')
            || url.startsWith('https://chromewebstore.google.com');
    }

    /** Is the active tab one we can read? → { ok, reason } */
    ...
    async function extract() {
        ...
        // Extension pages and browser internal pages can't be scripted
    ...
    return { extract, extractAsSnippets, isRestrictedUrl, canExtractActiveTab };
})();
