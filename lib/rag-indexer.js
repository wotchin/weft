...
const RAGIndexer = (() => {
    ...
    async function computeSessionRevision(snippets, options = {}) {
        ...
        throwIfAborted(signal);
        return `rag-v3-${items.length}-${(state.fnv >>> 0).toString(16).padStart(8, '0')}-${(state.djb >>> 0).toString(16).padStart(8, '0')}`;
    }
    ...
    function withCrossContextLock(sessionName, task, signal) {
        if (typeof navigator !== 'undefined' && navigator.locks?.request) {
            ...
            return navigator.locks.request(`weft-rag-index-v3:${sessionName}`, lockOptions, task);
        }
        return task();
    }
