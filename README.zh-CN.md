...
- **Smart Read** 抽取文章要点建立新 Session；每条摘要在保存前都会反查渲染后的源文，找不到出处的丢弃，防幻觉片段入库。
...
1. 对长文运行 **Smart Read** 建立聚焦 Session；开 **Show on Page** 可在原网页查看已采集段落。
...
| 抽取 | `lib/page-extractor.js`、`lib/smart-read.js`、`lib/highlighter.js` | 来源校验、页面高亮 |
